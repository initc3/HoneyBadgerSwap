...
func AddLiquidity(network string, conn *ethclient.Client, auth *bind.TransactOpts, tokenA common.Address, tokenB common.Address, amtA *big.Int, amtB *big.Int) {
...
func RemoveLiquidity(network string, conn *ethclient.Client, auth *bind.TransactOpts, tokenA common.Address, tokenB common.Address, amt *big.Int) {
...
func TradePrep(network string, conn *ethclient.Client, auth *bind.TransactOpts) (int64, int64) {
	hbswapInstance, err := hbswap.NewHbSwap(HbswapAddr[network], conn)
	if err != nil {
		log.Fatal(err)
	}
...
func Trade(network string, conn *ethclient.Client, auth *bind.TransactOpts, tokenA common.Address, tokenB common.Address, idxA *big.Int, idxB *big.Int, maskedA *big.Int, maskedB *big.Int) {
	hbswapInstance, err := hbswap.NewHbSwap(HbswapAddr[network], conn)
	if err != nil {
		log.Fatal(err)
	}
...
func GetBalance(network string, conn *ethclient.Client, tokenAddr common.Address, user common.Address) *big.Int {
	hbswapInstance, err := hbswap.NewHbSwap(HbswapAddr[network], conn)
	if err != nil {
		fmt.Printf("here")
		log.Fatal(err)
