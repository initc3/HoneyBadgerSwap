...
from ratel.genfiles.python.fabcar import monitorRecordShipment, monitorQueryPositions, monitorQueryNumber, \
    monitorQueryFirst
...
if __name__ == '__main__':
    ...
    tasks = [preprocessing(db, contract, serverID),
             server.http_server(),
             monitorRecordShipment(web3, db, serverID, contract, confirmation, account),
             monitorQueryPositions(web3, db, serverID, contract, confirmation, account),
             monitorQueryNumber(web3, db, serverID, contract, confirmation, account),
             monitorQueryFirst(web3, db, serverID, contract, confirmation, account),
            ]
